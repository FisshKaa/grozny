...
const WP = {
  ak:   { n:'АК-74', r:300, rof:1, acc:.34, di:5, dv:0, pen:2, supp:6, k:'small' },
  akm:  { n:'АКМ', r:280, rof:1, acc:.32, di:5.5, dv:0, pen:2, supp:6, k:'small' },
  rpk:  { n:'РПК-74', r:420, rof:1.1, acc:.28, di:7, dv:0, pen:3, supp:11, k:'mg' },
  pkm:  { n:'ПКМ', r:520, rof:1.2, acc:.3, di:9, dv:0, pen:5, supp:15, k:'mg' },
  pkt:  { n:'ПКТ', r:560, rof:1.1, acc:.3, di:8, dv:0, pen:5, supp:12, k:'mg' },
  ...
  kpvt: { n:'КПВТ 14,5 мм', r:800, rof:1.5, acc:.33, di:13, dv:12, pen:32, supp:18, k:'mg' },
  ...
  amur: { n:'4×23 мм АЗП-23', r:1500, rof:1, acc:.4, di:26, dv:10, pen:25, supp:40, k:'mg', he:true, splash:6 , bd:6 },
  zu23: { n:'ЗУ-23-2', r:1500, rof:1.2, acc:.35, di:20, dv:8, pen:25, supp:30, k:'mg', he:true, splash:5 , bd:5 }
};
...
const UT = {
  f_msv:{ side:'fed', name:'Мотострелковое отделение', icon:'inf', cls:'inf', mc:'foot', men:8, hp:80, speed:4, vision:320, acc:.85, cost:25, limit:12,
    w:['ak','rpk',['rpg7',{ammo:4}]], desc:'Основная пехота группировки. В декабре 1994 года многие подразделения собирали наспех из срочников разных частей, без боевого слаживания.' },
  f_vdv:{ side:'fed', name:'Отделение ВДВ', icon:'inf', cls:'inf', mc:'foot', men:7, hp:70, speed:4.3, vision:340, acc:1.05, cost:35, limit:8,
    w:['ak','pkm',['rpg26',{ammo:3}]], desc:'Десантники были подготовлены лучше мотострелков и чаще действовали штурмовыми группами.' },
  f_rec:{ side:'fed', name:'Разведгруппа', icon:'recon', cls:'inf', mc:'foot', men:5, hp:50, speed:4.6, vision:460, stealth:.7, acc:1, cost:30, limit:4,
    w:['ak',['rpg26',{ammo:2}]], desc:'Видит дальше и заметна меньше обычной пехоты, но огневой мощи мало. Нужна технике как глаза.' },
  ...
  c_opl:{ side:'chr', name:'Отряд ополчения', icon:'inf', cls:'inf', mc:'foot', men:6, hp:60, speed:4.2, vision:320, acc:.8, cost:15, limit:14,
    w:['akm',['rpg7',{ammo:2}]], desc:'Добровольцы. Знают город и мотивированы, но плохо вооружены и почти не обучены.' },
  c_rpg:{ side:'chr', name:'Гранатомётная группа', icon:'inf', cls:'inf', mc:'foot', men:4, hp:40, speed:4.4, vision:320, stealth:.85, acc:1, cost:35, limit:10,
    w:['akm','pkm',['rpg7',{ammo:8}]], desc:'Типичная ячейка обороны Грозного: гранатомётчики, пулемётчик и автоматчики. Бьют сверху по крышам и бортам и сразу меняют позицию.' },
  c_vet:{ side:'chr', name:'Штурмовая группа ветеранов', icon:'inf', cls:'inf', mc:'foot', men:6, hp:60, speed:4.5, vision:340, stealth:.9, acc:1.15, cost:45, limit:6,
    w:['akm','pkm',['rpg7',{ammo:5}]], desc:'Бойцы с опытом войны в Абхазии 1992–1993 годов. Самые подготовленные подразделения чеченской стороны.' },
  ...
  c_pk:{ side:'chr', name:'Пулемётный расчёт', icon:'mg', cls:'inf', mc:'foot', men:3, hp:30, speed:4, vision:320, acc:1, cost:20, limit:6,
    w:['pkm','akm'], desc:'Расчёт ПКМ. Прижимает пехоту и отрезает её от техники.' },
  ...
  f_sap:{ side:'fed', name:'Инженерно-сапёрное отделение', icon:'eng', cls:'inf', mc:'foot', men:6, hp:60, speed:4, vision:320, acc:.9, cost:30, limit:3, sapper:true,
    w:['ak',['rpg26',{ammo:1}]], desc:'Находит мины в радиусе 70 м, снимает их и разбирает баррикады. Остановите сапёров рядом с целью, дальше они работают сами.' },
...
const AB = {
  ...
  chr:{ name:'Миномёт 82 мм', charges:2, cost:20, cd:60, delay:6, shells:5, scatter:40, r:14, dmg:28, pen:10, bdmg:48,
...
function infilOk(g, side, type, x, y){
  ...
  const m = g.map; if (x < 0 || y < 0 || x >= m.W*CELL || y >= m.H*CELL) return false;
  const i = cellIdx(m, x, y); if (!m.res[i] || (m.t[i] !== T.BLD && m.t[i] !== T.RUBBLE)) return false;
...
const SCEN = {
  ny95:{
    ...
    decks:{ fed:['f_msv','f_sap','f_vdv','f_rec','f_snp','f_cmd','f_btr','f_bmp','f_t72','f_t80','f_zsu','f_ural'],
            chr:['c_opl','c_rpg','c_vet','c_snp','c_pk','c_t72','c_btr','c_zu','c_sup'] },
  ...
  aug96:{
    ...
    decks:{ fed:['f_msv','f_sap','f_vdv','f_snp','f_cmd','f_btr','f_bmp','f_t72b','f_zsu','f_ural'],
            chr:['c_opl','c_rpg','c_vet','c_snp','c_pk','c_zu','c_sup'] },
  ...
  yar96:{
    ...
    decks:{ fed:['f_msv','f_sap','f_rec','f_snp','f_cmd','f_btr','f_bmp','f_t72b','f_zsu'],
            chr:['c_opl','c_rpg','c_vet','c_snp','c_pk','c_zu','c_sup'] },
...
function assignFloors(m, seed, kind){
  const R = mulberry(seed*31 + 5), W = m.W, fl = new Uint8Array(W*m.H), seen = new Uint8Array(W*m.H);
...
/* ================= ИГРА ================= */
function mkSide(s){ return { pts:s.points, inc:s.income, score:0, lostVal:0, lostMen:0, lostVeh:0, civK:0, used:{}, eng:{}, cd:0, cd2:0, cd3:0, ready:false }; }
...
function makeGame(scnId, seed, opts){
  ...
  const g = { scn:scnId, sc, seed, map, t:0, phase:'deploy', phaseT:sc.deployTime, units:[], byId:new Map(), nextId:1,
    ...
    ev:[], shells:[], artyMarks:[], arrivals:[], bldHits:0, bldDown:0, civs:[], noise:[], civDead:0, civEvac:0, result:null, visT:0, delayed:[], fires:new Map(), entry:JSON.parse(JSON.stringify(sc.entry)),
...
function ownUnits(g, side, ids, withCarried){ const out = []; for (const id of ids || []){ const u = g.byId.get(id); if (u && !u.dead && u.side === side && (withCarried || !u.carrier)) out.push(u); } return out; }
...
function applyCommand(g, side, c){
  ...
  switch (c.c){
    case 'deploy': {
      ...
      S.pts -= d.cost; S.used[c.type] = (S.used[c.type] || 0) + 1; spawnUnit(g, c.type, side, c.x, c.y); return;
    }
    case 'undeploy': {
    ...
    case 'eng': {
      ...
      if (S.pts < E.cost || (S.eng[c.k] || 0) >= E.lim[side] || !engOk(g, side, c.x, c.y)) return;
    ...
    case 'buy': {
      ...
      if (c.x !== undefined){
        ...
        S.pts -= d.cost; S.used[c.type] = (S.used[c.type] || 0) + 1; g.arrivals.push({ type:c.type, side, at:g.t + ARRIVE + 3, x:p.x, y:p.y }); return;
      }
      S.pts -= d.cost; S.used[c.type] = (S.used[c.type] || 0) + 1; g.arrivals.push({ type:c.type, side, at:g.t + ARRIVE }); return;
    }
    case 'afire': {
      if (g.phase !== 'battle') return;
    ...
    case 'move': { if (g.phase !== 'battle') return; const us = ownUnits(g, side, c.ids); if (us.length) formationMove(g, us, c.x, c.y, !!c.fast, !!c.queue); if (!c.queue) for (const u of us){ u.focus = 0; u.afire = null; } return; }
    ...
    case 'stop': for (const u of ownUnits(g, side, c.ids)){ u.path = null; u.dest = null; u.focus = 0; u.fast = false; u.board = 0; u.afire = null; u.q = null; } return;
    case 'tunnel': {
...
function fireAt(g, u, w, ws, b){
  ...
  if (db.cls === 'inf'){
    const tb = terrAt(g.map, b.x, b.y);
    let c = 1; if (g.map.obst[cellIdx(g.map, b.x, b.y)]) c = .6; if (tb === T.BLD) c = w.he ? .7 : .35; else if (tb === T.RUBBLE) c = w.he ? .65 : .5; else if (tb === T.FOREST) c = .6; else if (tb === T.ROCK) c = .5;
    p *= c;
  ...
  g.ev.push({ e:'s', a:[Math.round(u.x), Math.round(u.y), Math.round(b.x+mx), Math.round(b.y+my)], k:w.k, sid:u.id, tid:b.id });
...
function areaFire(g, u, d, w, ws){
  ...
  if (!los(g.map, u.x, u.y, A.x, A.y)) return false;
  ...
  u.fired = g.t; if (isFinite(ws.ammo)) ws.ammo--;
  g.ev.push({ e:'s', a:[Math.round(u.x), Math.round(u.y), Math.round(ix), Math.round(iy)], k:w.k, sid:u.id, tid:0 });
...
function startRound2(g){
  const sc = g.sc;
  g.round = 2; g.phase = 'deploy'; g.phaseT = sc.deployTime;
  ...
  g.shells = []; g.artyMarks = []; g.arrivals = []; g.mines = []; g.smokes = []; g.flares = []; g.delayed = []; g.fires = new Map();
...
function step(g, dt){
  ...
  if (g.phase === 'deploy'){
    g.phaseT -= dt;
    if (g.phaseT <= 0 || (g.sides.fed.ready && g.sides.chr.ready)){ g.phase = 'battle'; g.phaseT = g.sc.duration; g.ev.push({ e:'battle' }); updateVis(g); }
    return;
  }
  g.t += dt; g.phaseT -= dt;
  for (const s of SIDES){ const S = g.sides[s]; S.pts += S.inc*dt; if (S.cd > 0){ S.cd -= dt; if (S.cd <= 0) S.ch = AB[s].charges || 1; } else if (S.ch !== undefined && S.ch <= 0) S.ch = AB[s].charges || 1; if (S.ch === undefined) S.ch = AB[s].charges || 1; if (S.cd2 > 0) S.cd2 -= dt; if (S.cd3 > 0) S.cd3 -= dt; }
  ...
  for (const a of g.arrivals.filter(a => a.at <= g.t)){
    ...
    if (ci >= 0){ const p = cellCenter(g.map, ci); const nu = spawnUnit(g, a.type, a.side, p.x, p.y); g.ev.push({ e:'arr', side:a.side, type:a.type, id:nu.id, x:Math.round(p.x), y:Math.round(p.y), inf:a.x !== undefined ? 1 : 0 }); }
  }
  ...
  if (g.engT <= 0){
    ...
    for (const u of g.units){
      ...
      u.work += .5;
      if (u.work >= tgt.need){
        if (tgt.k === 'mine'){ const j = g.mines.findIndex(n => n.id === tgt.id); if (j >= 0){ const n = g.mines[j]; g.mines.splice(j, 1); g.ev.push({ e:'demine', side:u.side, x:Math.round(n.x), y:Math.round(n.y) }); } }
...
function encodeSnap(g, ev){
  return { ph:g.phase, pt:Math.round(g.phaseT*10)/10, t:Math.round(g.t*100)/100,
    ...
    u:g.units.map(u => [u.id, TI[u.type], u.side === 'fed' ? 0 : 1, Math.round(u.x), Math.round(u.y), Math.round(u.h*100), Math.round(u.hp), Math.round(u.supp),
      ...
      u.dest ? Math.round(u.dest.x) : -1, u.dest ? Math.round(u.dest.y) : -1, u.w.map(w => isFinite(w.ammo) ? w.ammo : -1), Math.round(u.ta*100), u.carrier, u.smk, (u.immob?1:0)|(u.workT?2:0)|(u.board?4:0)|(u.workT && u.workT.k === 'mine'?8:0), u.afire ? Math.round(u.afire.x) : -1, u.afire ? Math.round(u.afire.y) : -1, (u.nocomm?1:0)|(u.under?2:0), u.pend ? Math.max(1, Math.ceil(u.pend - g.t)) : 0, u.q ? u.q.map(p => [Math.round(p.x), Math.round(p.y)]) : 0, (u.ambush?1:0)|(u.inFire?2:0)]),
    fr:[...g.fires.keys()],
...
function decodeSnap(s){
  return { phase:s.ph, phaseT:s.pt, t:s.t, sides:s.S, objs:s.o, bh:s.bh, res:s.res, marks:s.a, civDead:s.cd, civEvac:s.ce, bldDown:s.bdn,
    ...
    units:s.u.map(a => ({ id:a[0], type:TK[a[1]], side:a[2] ? 'chr' : 'fed', x:a[3], y:a[4], h:a[5]/100, hp:a[6], supp:a[7],
      ...
      dest:a[9] >= 0 ? { x:a[9], y:a[10] } : null, ammo:a[11], ta:a[12]/100, carrier:a[13], smk:a[14], immob:!!(a[15]&1), working:(a[15]&2) ? ((a[15]&8) ? 'mine' : 'barr') : null, boarding:!!(a[15]&4), afire:a[16] >= 0 ? { x:a[16], y:a[17] } : null, nocomm:!!(a[18]&1), under:!!(a[18]&2), pend:a[19] || 0, q:a[20] || null, ambush:!!(a[21]&1), inFire:!!(a[21]&2) })), fires:s.fr || [] };
}

if (typeof module !== 'undefined') module.exports = { cellarPath, cellarReach, hasComm, UT, SCEN, AB, AB2, ENG, T, CELL, TK, spawnUnit, infilOk, engOk, makeGame, step, applyCommand, aiStep, encodeSnap, decodeSnap, findPath, buildMap, los };
