...
const APP = { mode:null, side:null, g:null, sc:null, map:null, mapCv:null, view:null, byId:new Map(), prev:new Map(), viewT:0,
  ...
  keys:{}, ai:{}, lobby:{ scn:'ny95', hostSide:'chr', guest:false }, loop:null, netEv:[], lastSnap:0, drag:null, pan:null,
...
const pseudoG = () => ({ sc:APP.sc, map:APP.map, phase:APP.view.phase, units:APP.view.units.filter(u => u.side === APP.side || u.vis[APP.side]) });
const infilOkC = (type, x, y) => APP.sc && APP.view && infilOk(pseudoG(), APP.side, type, x, y);
const engOkC = (x, y) => APP.sc && engOk(pseudoG(), APP.side, x, y);
...
function netClose(){ try { NET.conn && NET.conn.close(); } catch(e){} try { NET.peer && NET.peer.destroy(); } catch(e){} NET.conn = null; NET.peer = null; }
function hasPeer(){ return typeof Peer !== 'undefined'; }
const netOpen = () => NET.conn && NET.conn.open;
...
function hostLobby(){
  ...
  netClose(); APP.mode = 'host'; APP.lobby = { scn:APP.lobby.scn, hostSide:APP.lobby.hostSide || 'chr', guest:false, code:genCode() };
  ...
  p.on('open', () => { setLobbyStatus('Ждём соперника. Отправьте ему код.'); renderLobby(); });
  p.on('connection', c => {
    if (netOpen()){ c.on('open', () => { c.send({ t:'full' }); setTimeout(() => c.close(), 300); }); return; }
    NET.conn = c;
    c.on('open', () => { APP.lobby.guest = true; setLobbyStatus('Соперник подключился.'); sendLobby(); renderLobby(); });
...
function sendLobby(){ if (netOpen()) NET.conn.send({ t:'lobby', scn:APP.lobby.scn, hostSide:APP.lobby.hostSide, rounds:APP.lobby.rounds, tod:APP.lobby.tod }); }
function onChat(m){ if (typeof m.m !== 'string') return; const who = SIDE_NAME[m.side] || 'Соперник'; addLog(who + ': ' + m.m.slice(0, 200), 'chat ' + (m.side || ''), null, 'chat'); sound('hit', .2); }
function onHostData(m){
  if (!m || !m.t) return;
  if (m.t === 'hello') sendLobby();
  else if (m.t === 'side'){ if (!APP.g){ APP.lobby.hostSide = other(m.side); sendLobby(); renderLobby(); } }
  else if (m.t === 'cmd' && APP.g) applyCommand(APP.g, other(APP.side), m.c);
  else if (m.t === 'chat') onChat(m);
}
function onGuestData(m){
  if (!m || !m.t) return;
  if (m.t === 'full'){ setJoinStatus('В этом лобби уже есть второй игрок.'); return; }
  if (m.t === 'lobby'){
    APP.lobby.scn = m.scn; APP.lobby.hostSide = m.hostSide; APP.lobby.guest = true; APP.lobby.rounds = m.rounds || 1; APP.lobby.tod = m.tod || 'day';
    if (APP.sc){ stopGame(); }
    showScreen('lobby'); renderLobby(); setLobbyStatus('Вы в лобби. Сценарий выбирает создатель, сторону можно сменить.');
  } else if (m.t === 'start'){ startGame('guest', m.scn, m.seed, m.side, m.opts); }
  else if (m.t === 'snap' && APP.sc){ onSnap(m.s); }
  else if (m.t === 'chat') onChat(m);
}
function onDisconnect(){
  if (APP.mode === 'host' && !APP.g){ APP.lobby.guest = false; NET.conn = null; setLobbyStatus('Соперник отключился. Ждём нового подключения.'); renderLobby(); return; }
  if (APP.sc && !(APP.view && APP.view.phase === 'end')){ addLog('Соединение с соперником потеряно.', 'bad'); toast('Соединение с соперником потеряно.', 6000, 'bad'); }
...
function sendCmd(c){
  ...
  else if (APP.g) applyCommand(APP.g, APP.side, c);
}
function sendChat(text){
  ...
  if (netOpen()) NET.conn.send({ t:'chat', m:text, side:APP.side });
}
...
function startGame(mode, scn, seed, side, opts){
  opts = opts || {};
  ...
  APP.rec = { base:{ t:g.map.t.slice(), bst:g.map.bst.slice(), obst:g.map.obst.slice() }, frames:[], events:[], lastF:0, ended:false }; APP.replay = null; APP.mapCv = renderMapCanvas(g.map); APP.miniDirty = true;
  ...
  if (mode !== 'guest'){
    ...
    APP.loop = setInterval(() => {
      ...
      if (mode === 'host' && netOpen() && now - APP.lastSnap > 95){
        APP.lastSnap = now; s.ev = APP.netEv; APP.netEv = []; NET.conn.send({ t:'snap', s });
      }
    ...
    switch (e.e){
      ...
      case 'arr':
        if (e.side === me){
          ...
          addPing(e.x, e.y, REL.own, 9000, UT[e.type].name); APP.newUnits.set(e.id, now);
        }
        break;
      case 'obj':
...
function drawUnit(u, now, models, s){
  ...
  ctx.lineWidth = (sel ? 3 : 2)/z; ctx.strokeStyle = sel ? REL.sel : own ? REL.own : REL.en;
...
function replaySeek(t){
  ...
  m.t.set(B.t); m.bst.set(B.bst); m.obst.set(B.obst);
...
function frame(now){
  ...
  if (APP.placing && APP.placing.startsWith('eng:')){
    ...
    for (let yy = y0; yy <= y1; yy++) for (let xx = x0; xx <= x1; xx++){
      ...
      if (eng ? engOk(pg, me, cx, cy) : infilOk(pg, me, APP.placing, cx, cy)) ctx.fillRect(xx*CELL, yy*CELL, CELL, CELL);
    }
  }
  ...
  for (const u of shown) if (u.side === me && u.q && u.q.length && APP.sel.has(u.id)){
    ...
    ctx.fillStyle = 'rgba(240,235,210,.85)'; u.q.forEach(([qx, qy], k) => { ctx.beginPath(); ctx.arc(qx, qy, 3/z, 0, 7); ctx.fill(); ctx.font = `${10/z}px "PT Sans Narrow",sans-serif`; ctx.fillText(String(k + 2), qx + 5/z, qy - 4/z); });
  }
  ...
  if ((APP.targeting === 'tunnel' && APP.tunnelReach) || (APP.placing && me === 'chr' && UT[APP.placing] && UT[APP.placing].cls === 'inf')){
    ...
    if (APP.targeting === 'tunnel') for (const ci of APP.tunnelReach) mark(ci);
      ...
      const ci = yy*m.W + xx; if (infilOk(pg, me, APP.placing, xx*CELL + 10, yy*CELL + 10)) mark(ci); }
  }
  ...
  if (APP.placing && APP.mouse){
    ...
    if (APP.placing.startsWith('eng:')){
      const k = APP.placing.slice(4), ok = engOkC(w.x, w.y);
      ctx.globalAlpha = ok ? .85 : .3;
      if (k === 'barr'){ const cx = Math.floor(w.x/CELL)*CELL, cy = Math.floor(w.y/CELL)*CELL; ctx.fillStyle = '#6d6a63'; ctx.fillRect(cx, cy, CELL, CELL); ctx.strokeStyle = REL.own; ctx.lineWidth = 2/z; ctx.strokeRect(cx, cy, CELL, CELL); }
  ...
  if (APP.targeting && APP.mouse){
    const w = toWorld(APP.mouse.x, APP.mouse.y), R = APP.targeting === 'smoke' ? AB2[me].scatter*1.4 : APP.targeting === 'flare' ? FLARE.r : APP.targeting === 'afire' ? 25 : AB[me].scatter*1.6;
...
addEventListener('mouseup', e => {
  ...
  if (!d.box){
    ...
    const u = unitAt(e.clientX, e.clientY, u => u.side === me);
    if (!e.shiftKey) APP.sel.clear();
    if (u){ if (e.detail >= 2){ for (const x of viewUnits()) if (x.side === me && x.type === u.type){ const p = toScreen(x.rx, x.ry); if (p.x > 0 && p.y > 0 && p.x < cw && p.y < ch) APP.sel.add(x.id); } } else APP.sel.add(u.id); }
  } else {
    if (!e.shiftKey) APP.sel.clear();
    const x0 = Math.min(d.x0, d.x1), x1 = Math.max(d.x0, d.x1), y0 = Math.min(d.y0, d.y1), y1 = Math.max(d.y0, d.y1);
    for (const u of viewUnits()){ if (u.side !== me) continue; const p = toScreen(u.rx, u.ry - (u.lift || 0)); if (p.x >= x0 && p.x <= x1 && p.y >= y0 && p.y <= y1) APP.sel.add(u.id); }
  }
  updateSel();
});
function rightClick(e){
  ...
  const ids = [...APP.sel].filter(id => APP.byId.has(id) && APP.byId.get(id).side === me);
  const w = pickW(e.clientX, e.clientY);
  const own = unitAt(e.clientX, e.clientY, u => u.side === me);
  ...
  if (v.phase === 'deploy'){
    ...
    if (ids.length === 1){ sendCmd({ c:'redeploy', id:ids[0], x:w.x, y:w.y }); return; }
...
addEventListener('keydown', e => {
  ...
  if (e.code === 'KeyA' && e.ctrlKey){ e.preventDefault(); for (const u of APP.view.units) if (u.side === APP.side) APP.sel.add(u.id); updateSel(); }
  ...
  if (/^Digit[0-9]$/.test(e.code)){
    ...
    const gr = (APP.groups[n] || []).filter(id => APP.byId.has(id) && APP.byId.get(id).side === APP.side);
    if (!gr.length) return;
    const now = performance.now();
  ...
  if (e.code === 'KeyI' || e.code === 'KeyC'){
    ...
    for (const u of viewUnits()){ if (u.side !== APP.side || (UT[u.type].cls === 'inf') !== inf) continue; const p = toScreen(u.rx, u.ry - (u.lift || 0)); if (p.x > 0 && p.y > 0 && p.x < cw && p.y < ch) APP.sel.add(u.id); }
    updateSel(); updateHud();
  }
...
function toggleAbility(kind){
  ...
  const cd = kind === 'smoke' ? S.cd2 : kind === 'flare' ? S.cd3 : (S.ch > 0 ? 0 : S.cd), cost = kind === 'smoke' ? AB2[APP.side].cost : kind === 'flare' ? FLARE.cost : AB[APP.side].cost;
...
function selOwn(){ return [...APP.sel].map(id => APP.byId.get(id)).filter(u => u && u.side === APP.side && !u.carrier); }
function unitSmoke(){
...
function buildDeck(){
  ...
  if (APP.view.phase === 'deploy'){
    ...
    if (APP.side !== APP.sc.defender){ const n = document.createElement('p'); n.className = 'deck-n'; n.textContent = APP.side === 'fed' ? 'Мины и баррикады ставит обороняющийся. У вас есть сапёры: они находят мины за 70 м и снимают их.' : 'Мины и баррикады ставит обороняющийся. Зато ваша пехота может выйти из любого жилого дома вдали от противника.'; box.appendChild(n); }
    else for (const ek of Object.keys(ENG)){
...
function updateHud(){
  ...
  $('#hint').textContent = APP.placing && v.phase === 'deploy' && me === 'chr' && UT[APP.placing] && UT[APP.placing].cls === 'inf' ? 'Пехоту можно ставить в своей зоне или в любом подсвеченном жилом доме. Shift+щелчок — несколько.'
    ...
    : v.phase === 'deploy' ? 'Выберите отряд в колоде и поставьте в своей зоне. Выбранную пехоту можно сразу посадить в БТР или БМП правым щелчком по машине.'
...
function updateSel(){
  ...
  const us = v.units.filter(u => APP.sel.has(u.id) && u.side === APP.side && !u.carrier);
...
$('#end-lobby').onclick = () => { stopGame(); showScreen('lobby'); renderLobby(); sendLobby(); setLobbyStatus(APP.lobby.guest ? 'Соперник в лобби. Можно начинать новый бой.' : 'Ждём соперника.'); };
...
function renderLobby(){
  ...
  const mySide = m === 'guest' ? other(L.hostSide) : L.hostSide;
  ...
  for (const sd of SIDES){
    const b = $('#l-side-' + sd); const who = sd === mySide ? 'вы' : (m === 'solo' ? 'бот' : (m === 'host' ? (L.guest ? 'соперник' : 'свободно') : 'создатель'));
    b.classList.toggle('on', sd === mySide); b.querySelector('span').textContent = who;
    b.onclick = () => {
      if (m === 'guest'){ if (netOpen()) NET.conn.send({ t:'side', side:sd }); }
      else { L.hostSide = sd; renderLobby(); sendLobby(); }
    };
  }
  ...
  const start = $('#l-start');
  start.hidden = m === 'guest'; start.disabled = m === 'host' && !L.guest;
  start.textContent = m === 'host' && !L.guest ? 'Ждём соперника' : 'Начать бой';
}
$('#l-start').onclick = () => {
  const L = APP.lobby, seed = Math.floor(Math.random()*1e6);
  const opts = { rounds:L.rounds, tod:L.tod };
  if (APP.mode === 'solo'){ startGame('solo', L.scn, seed, L.hostSide, opts); return; }
  if (!netOpen()){ setLobbyStatus('Соперник не подключён.'); return; }
  NET.conn.send({ t:'start', scn:L.scn, seed, side:other(L.hostSide), opts });
  startGame('host', L.scn, seed, L.hostSide, opts);
};
