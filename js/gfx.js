...
function drawCell(x, map, i){
  ...
  if (v === T.ROAD){
    ...
    // мусор и подбитые гражданские машины
    if (hr(i, 9) < .035){ x.save(); x.translate(px + 6 + hr(i, 10)*8, py + 6 + hr(i, 11)*8); x.rotate(hr(i, 12)*3); x.fillStyle = hr(i, 13) < .5 ? '#4a3a30' : '#5c5a55'; x.fillRect(-4, -2, 8, 4); x.fillStyle = '#221f1c'; x.fillRect(-2.5, -1.5, 3, 3); x.restore(); }
  }
  else if (v === T.YARD){
    const r = hr(i, 30);
    if (r < .07){ // дерево
    ...
    } else if (r < .185){ // сгоревшая машина во дворе
      x.fillStyle = '#3d342c'; x.fillRect(px + 5, py + 8, 9, 4.5); x.fillStyle = '#1f1c19'; x.fillRect(px + 7, py + 9, 4, 2.5);
    }
