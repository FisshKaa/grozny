...
function drawCell(x, map, i){
  ...
  if (v === T.ROAD){
    const isR = j => j === T.ROAD || j === T.BRIDGE;
    const sw = wn ? '#cfd1cd' : '#c9c1a6', curb = wn ? '#8f928f' : '#8c8570';
    const n = at(0,-1), s = at(0,1), w = at(-1,0), e = at(1,0);
    x.fillStyle = sw;
    if (!isR(n)) x.fillRect(px, py, CELL, 4); if (!isR(s)) x.fillRect(px, py + CELL - 4, CELL, 4);
    if (!isR(w)) x.fillRect(px, py, 4, CELL); if (!isR(e)) x.fillRect(px + CELL - 4, py, 4, CELL);
    x.fillStyle = curb;
    if (!isR(n)) x.fillRect(px, py + 4, CELL, .8); if (!isR(s)) x.fillRect(px, py + CELL - 4.8, CELL, .8);
    if (!isR(w)) x.fillRect(px + 4, py, .8, CELL); if (!isR(e)) x.fillRect(px + CELL - 4.8, py, .8, CELL);
    // осевая разметка между двумя полосами
    x.fillStyle = wn ? 'rgba(230,232,230,.55)' : 'rgba(245,240,220,.75)';
    const vert = isR(n) && isR(s), hor = isR(w) && isR(e);
    if (vert && !hor && isR(e) !== isR(w)){ if (isR(e) && Y % 2 === 0) x.fillRect(px + CELL - .6, py + 4, 1.2, 10); }
    else if (hor && !vert && isR(s) !== isR(n)){ if (isR(s) && X % 2 === 0) x.fillRect(px + 4, py + CELL - .6, 10, 1.2); }
    if (!isR(n) && !isR(w) && hr(i, 7) < .5 || (!isR(e) && hr(i, 8) < .12)){ x.fillStyle = '#3a3833'; x.fillRect(px + (isR(w) ? CELL - 3 : 1.5), py + 1.5, 1.4, 1.4); }
    // мусор на асфальте
    if (hr(i, 9) < .05){ x.fillStyle = wn ? 'rgba(90,95,95,.35)' : 'rgba(60,55,45,.35)'; x.fillRect(px + 4 + hr(i, 10)*10, py + 4 + hr(i, 11)*10, 2 + hr(i, 12)*3, 1.2); }
  }
  else if (v === T.YARD){
    const r = hr(i, 30);
    if (r < .07){ // дерево
      x.fillStyle = 'rgba(0,0,0,.18)'; x.beginPath(); x.arc(px + 12, py + 12, 5, 0, 7); x.fill();
      x.fillStyle = wn ? '#4f5f52' : '#7d8f63'; x.beginPath(); x.arc(px + 10, py + 10, 5, 0, 7); x.fill();
      x.fillStyle = wn ? '#e8ece9' : '#93a476'; x.beginPath(); x.arc(px + 9, py + 9, 2.4, 0, 7); x.fill();
    } else if (r < .11){ // гаражи
      x.fillStyle = wn ? '#9fa3a4' : '#77766c'; x.fillRect(px + 2, py + 5, 16, 8);
      x.fillStyle = wn ? '#e3e6e5' : '#8b8a80'; x.fillRect(px + 2, py + 5, 16, 2);
      x.fillStyle = '#3a3832'; for (let k = 0; k < 3; k++) x.fillRect(px + 3 + k*5.3, py + 10, 4, 3);
    } else if (r < .14){ // забор
      x.strokeStyle = wn ? '#7f8480' : '#6d6656'; x.lineWidth = .8; x.beginPath(); x.moveTo(px, py + 2); x.lineTo(px + CELL, py + 2); x.stroke();
    } else if (r < .16){ // детская площадка / скамейки
      x.fillStyle = '#6a5a3e'; x.fillRect(px + 4, py + 8, 6, 1.4); x.fillRect(px + 12, py + 12, 5, 1.4);
    }
    if (hr(i, 31) < .3){ x.fillStyle = wn ? 'rgba(170,175,172,.35)' : 'rgba(110,100,80,.18)'; x.fillRect(px + hr(i, 32)*14, py + hr(i, 33)*14, 4 + hr(i, 34)*4, 2); }
  }
...
const WALLS = ['#8c8a80', '#8a5a44', '#a8966a', '#b3ab98', '#7d8584', '#94705a'];
const ROOFS = ['#9a9481', '#8a8578', '#6f6b62', '#a39c86'];
const TINS = ['#5f7a5a', '#8a4a3a', '#7b7d79', '#6a5a7a', '#9a7b4a'];
function bldStyle(map, i){
  const b = map.bid ? map.bid[i] : 0, fl = map.fl ? map.fl[i] : 1, k = hr(b, 17);
  const wall = fl <= 2 ? WALLS[1 + Math.floor(hr(b, 3)*5)] : fl >= 9 ? (k < .7 ? WALLS[0] : WALLS[4]) : WALLS[Math.floor(k*WALLS.length)];
  return { wall, roof:ROOFS[Math.floor(hr(b, 5)*ROOFS.length)], tin:TINS[Math.floor(hr(b, 7)*TINS.length)],
    pitched:fl <= 2, panel:fl >= 5 && (wall === WALLS[0] || wall === WALLS[4]), brick:wall === WALLS[1] || wall === WALLS[5], balc:fl >= 5 && hr(b, 9) < .7, bitumen:hr(b, 11) < .5 };
}
function drawTallCell(x, map, i){
  ...
  if (exposed){ drawExposedWall(x, map, i, py + CELL - H, H, px, py, fl); }
    ...
    const st = bldStyle(map, i);
    const wall = !res ? (wn ? '#8d8f8b' : '#57503f') : wn ? shade(st.wall, 1.12) : st.wall;
    ...
    for (let f = 0; f < fl; f++){
      ...
      for (let k = 0; k < 4; k++){
        ...
        if (st.balc && f > 0 && k % 2 === (X % 2) && !broken){ x.fillStyle = shade(wall, 1.18); x.fillRect(px + 1 + k*4.6, wy + 1.5, 3.6, .7); }
      }
    }
    if (st.panel && res){ x.strokeStyle = 'rgba(0,0,0,.12)'; x.lineWidth = .5; for (let f = 1; f < fl; f++){ const ly = py + CELL - f*FLOOR_M; if (ly < top) break; x.beginPath(); x.moveTo(px, ly); x.lineTo(px + CELL, ly); x.stroke(); } }
    if (st.brick && res){ x.fillStyle = 'rgba(0,0,0,.08)'; for (let yy = top + 1; yy < top + hh; yy += 1.6) x.fillRect(px, yy, CELL, .35); }
    if (res && fl >= 4 && (X + map.bid[i]) % 3 === 0){ x.fillStyle = '#2a2722'; x.fillRect(px + 7, py + CELL - 2.6, 5, 2.4); x.fillStyle = shade(wall, 1.2); x.fillRect(px + 6, py + CELL - 3.1, 7, .7); }
  ...
  // крыша
  const ry = py - H;
  const st2 = bldStyle(map, i);
  let roof = wn ? (res ? (dmg ? '#bfc0bc' : '#e2e5e4') : (dmg ? '#a7a9a6' : '#c6c9c8')) : res ? (dmg ? shade(st2.roof, .87) : st2.roof) : (dmg ? '#5d5647' : '#6c6452');
  if (st2.pitched && res && !dmg){
    // скатная крыша частного дома: два ската и конёк
    const c1 = wn ? '#eef0ef' : st2.tin, c2 = wn ? '#c9ccca' : shade(st2.tin, .78);
    x.fillStyle = c1; x.fillRect(px, ry, CELL, CELL/2); x.fillStyle = c2; x.fillRect(px, ry + CELL/2, CELL, CELL/2);
    x.fillStyle = shade(st2.tin, .6); x.fillRect(px, ry + CELL/2 - .5, CELL, 1);
    if (hr(i, 5) < .3){ x.fillStyle = '#6b3d2e'; x.fillRect(px + 4 + hr(i, 6)*10, ry + 3, 2.4, 3); }
  } else { x.fillStyle = roof; x.fillRect(px, ry, CELL, CELL); }
  if (!st2.pitched && res && !wn && st2.bitumen){ x.fillStyle = 'rgba(0,0,0,.07)'; x.fillRect(px + 1, ry + 1, CELL - 2, CELL - 2); }
...
const MODEL_DEF = {
  ...
  // БМП-2: низкий корпус с длинным клиновидным ребристым лбом, башня посередине с тонкой длинной 30-мм пушкой и ПТУР на крыше башни, кормовые двери
  ifv:{ hull:[ ...tracks(8.2, 4, 1.2, 1.6), [-7.8, 7.6, -2.8, 2.8, 0, 2, 'dark'],
      [-8.2, 2.4, -4, 4, 1.6, 3, 'hull'], [2.4, 8.4, -3.8, 3.8, 1.2, 2.3, 'light'],
      [3, 3.3, -3.6, 3.6, 2.3, 2.5, 'dark'], [4.6, 4.9, -3.6, 3.6, 2, 2.3, 'dark'], [6.2, 6.5, -3.6, 3.6, 1.7, 2, 'dark'],
      [4.4, 6.2, 1.2, 3.2, 2.3, 2.6, 'dark'], [2.6, 4, -3.4, -2, 2.3, 2.8, 'dark'],
      [-6.6, -5, -3.2, -1.2, 3, 3.3, 'dark'], [-6.6, -5, 1.2, 3.2, 3, 3.3, 'dark'], [-3.8, -2.2, -3.2, -1.2, 3, 3.3, 'dark'], [-3.8, -2.2, 1.2, 3.2, 3, 3.3, 'dark'],
      [-8.6, -8.2, -3, -.2, 1.6, 2.9, 'dark'], [-8.6, -8.2, .2, 3, 1.6, 2.9, 'dark'] ],
    tx:0, tz:3, barrel:11, turret:[ [-2.3, 2.3, -2.1, 2.1, 0, 1.4, 'hull', 'e'], [-1.6, 1.4, -1.5, 1.5, 1.4, 2, 'light', 'e'], [-.6, .6, -2.1, 2.1, 1.3, 1.8, 'stripe'],
      [2.2, 11, -.3, .3, .6, 1.2, 'metal'], [9.6, 10.4, -.42, .42, .6, 1.2, 'dark'],
      [-1.4, 3.2, 1.9, 2.8, 1.6, 2.4, 'metal', 'e'], [-.8, .6, -1.3, -.3, 2, 2.5, 'dark'] ] },
  ...
  // БТР-80: «лодка» с острым носом, 8 колёс равномерно, маленькая башня с КПВТ впереди, дверь между 2-й и 3-й осью
  apc:{ hull:[ ...wheels([-6.3, -3, .6, 4], 3.7, 2), [-8.8, 6.4, -3, 3, 1, 3, 'hull'],
      [6.4, 8.4, -2.6, 2.6, 1, 2.6, 'light'], [8.4, 9.6, -1.6, 1.6, 1, 2.1, 'light'],
      [-8, 5.8, -2.4, 2.4, 3, 3.7, 'light'], [5.8, 7.6, -2, 2, 2.6, 3.2, 'dark'],
      [-1.8, -.6, -3.1, -2.9, 1.3, 2.7, 'dark'], [-1.8, -.6, 2.9, 3.1, 1.3, 2.7, 'dark'],
      [-8.4, -6, -2, 2, 3.7, 3.9, 'metal'], [-4.6, -3.4, -1.6, -.6, 3.7, 4, 'dark'], [-4.6, -3.4, .6, 1.6, 3.7, 4, 'dark'] ],
    tx:3, tz:3.7, barrel:8, turret:[ [-1.5, 1.5, -1.5, 1.5, 0, 1.2, 'hull', 'e'], [-1, 1, -1, 1, 1.2, 1.6, 'light', 'e'], [-.5, .5, -1.5, 1.5, .8, 1.4, 'stripe'], [1.4, 8, -.35, .35, .4, 1, 'metal'], [1.4, 4.6, .6, .9, .5, .9, 'metal'] ] },
  // Урал-4320: капотный грузовик — длинный капот перед кабиной
  ural:{ hull:[ ...wheels([-6, -3.2, 5.4], 3.3, 1.9), [-8.2, 8.8, -2.2, 2.2, .8, 1.6, 'dark'],
      [6.2, 9, -2.2, 2.2, 1.6, 3, 'hull'], [8.8, 9.2, -2, 2, 1.8, 2.8, 'dark'], [6.4, 8.8, -.3, .3, 3, 3.1, 'dark'],
      [3.6, 6.2, -2.9, 2.9, 1.6, 4, 'hull'], [5.8, 6.2, -2.5, 2.5, 2.9, 3.7, 'glass'], [3.6, 5.9, -2.8, 2.8, 4, 4.4, 'light'],
      [-8.2, 3.2, -3.1, 3.1, 1.6, 2.5, 'hull'], [-8.2, 3, -3, 3, 2.5, 4.2, 'tent'], [-8.2, 3, -.35, .35, 4.2, 4.4, 'dark'] ] },
  // КамАЗ: бескапотный, плоская кабина над двигателем
...
// гражданские машины: «Жигули», «рафик», автобус ЛиАЗ
const CAR_COLS = ['#8a2e28', '#d8d4c6', '#3c5a7a', '#b39a4a', '#4f6a44', '#6a6a6a'];
CAR_COLS.forEach((c, k) => { PAL['car' + k] = { hull:c, light:shade(c, 1.15), dark:'#2b2a27', track:'#222', metal:'#5a5b58', glass:'#8fa6ae', tent:c, era:c, wheel:'#1b1a18', stripe:c, wood:'#8d7a52', box:c }; });
PAL.crushed = { ...PAL.burnt, hull:'#3a3530', light:'#443d36' };
MODEL_DEF.car = { hull:[ ...wheels([-2.6, 2.8], 2.1, .9), [-4.6, 4.8, -2, 2, .5, 1.8, 'hull'], [-2.2, 1.8, -1.8, 1.8, 1.8, 2.9, 'glass'], [-2, 1.6, -1.6, 1.6, 2.9, 3.1, 'light'], [4.4, 4.8, -1.6, 1.6, .9, 1.4, 'dark'] ] };
MODEL_DEF.van = { hull:[ ...wheels([-3, 3.2], 2.2, 1), [-5, 5, -2.2, 2.2, .6, 4.2, 'hull'], [3.6, 5, -2, 2, 2.6, 3.6, 'glass'], [-4.8, 4.8, -2, 2, 4.2, 4.4, 'light'], [-5, 5, -2.25, 2.25, 2, 2.3, 'stripe'] ] };
MODEL_DEF.bus = { hull:[ ...wheels([-7, 6.6], 2.8, 1.2), [-11, 11, -2.8, 2.8, .7, 5.6, 'hull'], [-10.6, 10.6, -2.85, 2.85, 3, 4.6, 'glass'], [-10.8, 10.8, -2.6, 2.6, 5.6, 5.9, 'light'], [10.6, 11, -2.4, 2.4, 1.2, 4.6, 'glass'] ] };
MODEL_DEF.flat = { hull:[ [-4.8, 5, -2.3, 2.3, 0, .8, 'hull'], [-2.4, 2, -2, 2, .8, 1.1, 'dark'] ] };
function drawProp(c, pr, ws, z, models){
  if (!models){ c.fillStyle = pr.st === 1 ? '#2d2925' : pr.st === 2 ? '#3a3530' : CAR_COLS[pr.col]; c.save(); c.translate(pr.x, pr.y); c.rotate(pr.rot); const L = pr.kind === 'bus' ? 12 : 5; c.fillRect(-L, -2, L*2, 4); c.restore(); return; }
  const key = pr.st === 2 ? 'flat' : pr.kind, pal = pr.st === 1 ? 'burnt' : pr.st === 2 ? 'crushed' : 'car' + pr.col;
  drawVehicle(c, key, pal, pr.x, pr.y, pr.rot, pr.rot, ws*.95, 1);
}
const MODEL_OF = { f_t72b:'tank72b', f_cmd:'cmd', f_btr:'apc', f_bmp:'ifv', f_t72:'tank', f_t80:'tank80', f_zsu:'aa', f_ural:'ural', f_conv:'ural', c_t72:'tank', c_btr:'apc', c_zu:'tech', c_sup:'truck' };
function shade(col, f){
  let r, g, b;
  if (col[0] === '#'){ const n = parseInt(col.slice(1), 16); r = n >> 16; g = (n >> 8) & 255; b = n & 255; }
  else { const m = col.match(/\d+(\.\d+)?/g) || [0, 0, 0]; r = +m[0]; g = +m[1]; b = +m[2]; }
...
function drawIcon(c, x, y, s, d, own, alpha, selected){
  ...
  switch (d.icon){
    ...
    case 'gl': X(); c.moveTo(iw*.35, 0); c.arc(0, 0, iw*.35, 0, 7); break;
    case 'hmg': X(); c.moveTo(-iw*.25, -ih); c.lineTo(-iw*.25, ih); c.moveTo(iw*.25, -ih); c.lineTo(iw*.25, ih); break;
    case 'at': c.moveTo(-iw*.8, ih*.8); c.lineTo(0, -ih*.8); c.lineTo(iw*.8, ih*.8); break;
...
function sound(kind, vol){
  ...
  const now = performance.now(), gap = { jet:1500, pop:80, flare:200, small:35, mg:45, rocket:60, cannon:40, boom:25, whistle:120, collapse:150, hit:40 }[kind] || 30;
  ...
  switch (kind){
    ...
    case 'jet': sndNoise(4.5, 300, 1800, .35*v, 'bandpass', .8); sndNoise(4.5, 3000, 900, .12*v, 'highpass', .7, .4); break;
    case 'gren': sndNoise(.5, 900, 120, .45*v); break;
...
/* ---------- Самолёт Су-25 (вид сверху) ---------- */
function drawPlane(c, x, y, ang, s, shadow){
  c.save(); c.translate(x, y); c.rotate(ang); c.scale(s, s);
  const body = shadow ? 'rgba(0,0,0,.28)' : '#6d7466', dark = shadow ? 'rgba(0,0,0,0)' : '#4a4f45', light = shadow ? 'rgba(0,0,0,0)' : '#8a917f';
  c.fillStyle = body;
  c.beginPath(); c.moveTo(8, 0); c.lineTo(5, -.8); c.lineTo(-7, -.9); c.lineTo(-8, 0); c.lineTo(-7, .9); c.lineTo(5, .8); c.closePath(); c.fill(); // фюзеляж
  c.beginPath(); c.moveTo(1.5, -1); c.lineTo(-1.2, -7.2); c.lineTo(-2.6, -7.2); c.lineTo(-2, -1); c.closePath(); c.fill();          // крылья
  c.beginPath(); c.moveTo(1.5, 1); c.lineTo(-1.2, 7.2); c.lineTo(-2.6, 7.2); c.lineTo(-2, 1); c.closePath(); c.fill();
  c.beginPath(); c.moveTo(-5.6, -.8); c.lineTo(-7.2, -3); c.lineTo(-8, -3); c.lineTo(-7.4, -.8); c.closePath(); c.fill();        // стабилизатор
  c.beginPath(); c.moveTo(-5.6, .8); c.lineTo(-7.2, 3); c.lineTo(-8, 3); c.lineTo(-7.4, .8); c.closePath(); c.fill();
  if (!shadow){
    c.fillStyle = dark; c.fillRect(-4, -1.6, 5, .8); c.fillRect(-4, .8, 5, .8);                                               // мотогондолы
    c.fillStyle = light; c.beginPath(); c.ellipse(4.6, 0, 1.4, .5, 0, 0, 7); c.fill();                                          // фонарь
    c.fillStyle = dark; for (const yy of [-5.4, -3.6, 3.6, 5.4]) c.fillRect(-1.6, yy - .25, 1.8, .5);                          // подвески
  }
  c.restore();
}
function drawCrewGun(c, kind, x, y, ang, s){
  c.save(); c.translate(x, y); c.rotate(ang); c.scale(s, s);
  c.strokeStyle = '#1e1f1c'; c.fillStyle = '#3c4035'; c.lineWidth = .5;
  if (kind === 'zu'){ c.fillRect(-2.6, -1.8, 4.2, 3.6); c.fillStyle = '#23241f'; c.fillRect(-2, -2.6, 1.6, .8); c.fillRect(-2, 1.8, 1.6, .8);
    c.fillStyle = '#2d2f29'; c.fillRect(1.4, -.9, 6.6, .5); c.fillRect(1.4, .4, 6.6, .5); }
  else if (kind === 'nsv'){ c.fillRect(-1, -.5, 2, 1); c.fillStyle = '#23241f'; c.fillRect(1, -.25, 5.6, .5); c.beginPath(); c.moveTo(-1, 0); c.lineTo(-3, -1.8); c.moveTo(-1, 0); c.lineTo(-3, 1.8); c.moveTo(0, 0); c.lineTo(1.6, 1.8); c.stroke(); }
  else if (kind === 'spg'){ c.fillStyle = '#4a4f40'; c.fillRect(-3.4, -.6, 8.4, 1.2); c.fillStyle = '#23241f'; c.fillRect(-4, -.8, .8, 1.6); c.beginPath(); c.moveTo(0, 0); c.lineTo(-1.6, 2); c.moveTo(0, 0); c.lineTo(-1.6, -2); c.moveTo(0, 0); c.lineTo(1.8, 1.4); c.stroke(); }
  else if (kind === 'atgm'){ c.fillStyle = '#556048'; c.fillRect(-.8, -.45, 5, .9); c.fillStyle = '#2d2f29'; c.fillRect(-1.6, -.9, 1.4, 1.8); c.beginPath(); c.moveTo(-1, 0); c.lineTo(-2.6, 1.6); c.moveTo(-1, 0); c.lineTo(-2.6, -1.6); c.stroke(); }
  else if (kind === 'ags'){ c.fillStyle = '#3c4035'; c.fillRect(-1.4, -1, 3, 2); c.fillStyle = '#23241f'; c.fillRect(1.6, -.35, 2.6, .7); c.beginPath(); c.moveTo(-.6, 0); c.lineTo(-2.6, -1.8); c.moveTo(-.6, 0); c.lineTo(-2.6, 1.8); c.stroke(); }
  c.restore();
}
